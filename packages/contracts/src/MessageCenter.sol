...
contract MessageCenter {
    ...
    mapping(address user => mapping(address sender => Authorization auth))
        private authorizations;
    mapping(address sender => EnumerableSet.UintSet messageIds)
        private userMessages;
    mapping(address oracle => EnumerableSet.UintSet messageIds)
        private oracleMessages;
    mapping(address user => EnumerableSet.AddressSet senders)
        private userAuthorizedSenders;
    mapping(address sender => EnumerableSet.AddressSet users)
        private senderAuthorizedUsers;
    ...
    modifier noZeroAddress(address sender, address oracle) {
        if (sender == address(0) || oracle == address(0))
    ...
    function grantAuthorization(
        ...
        if (authorizations[msg.sender][_sender].isAuthorized)
            revert AuthorizationAlreadyGranted();

        userAuthorizedSenders[msg.sender].add(_sender);
        senderAuthorizedUsers[_sender].add(msg.sender); // Add this line
    ...
    function getUserAuthorizations()
        external
        view
        returns (Authorization[] memory authsInfo)
    {
        ...
        for (uint256 i = 0; i < senders.length; i++) {
            ...
            authsInfo[i] = Authorization({
                ...
                encryptedSymmetricKey: auth.encryptedSymmetricKey,
                iv: auth.iv
            });
        }
    ...
    {
        Authorization[] memory authInfos = new Authorization[](
